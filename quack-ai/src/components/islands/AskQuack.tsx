...
export default function AskQuack() {
  ...
  const [turns, setTurns] = useState<Turn[]>([
    { role: 'duck', text: 'Quack. (Ask me anything. I already know the answer.)' },
  ]);
  ...
  return (
    <div className="card flex h-full flex-col p-6">
      ...
      <div className="flex-1 space-y-3 overflow-y-auto font-mono text-sm" style={{ minHeight: '10rem' }}>
        {turns.map((turn, i) => (
          <p key={i} className={turn.role === 'duck' ? 'text-accent' : 'text-cream-dim'}>
            <span className="mr-2 select-none">{turn.role === 'duck' ? '>' : '$'}</span>
            {turn.text}
          </p>
        ))}
        {thinking && <p className="text-accent animate-pulse">&gt; reasoning…</p>}
      </div>
